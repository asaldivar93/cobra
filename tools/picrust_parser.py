...
class p_model():
    ...
    def parse_pathways_in_genus(self, pathways, gen_filter):
        gen_pwy = pathways.loc[:, gen_filter].dropna(how = 'all').index.to_list()
        not_minimal = ['added']
        while not_minimal:
            not_minimal = []
            for pwy in gen_pwy:
                pwy_id = '|' + pwy + '|'
                if pwy_id in all_pathways:
                    sub_pathways = metacyc_db[pwy_id].sub_pathways
                    if sub_pathways:
                        gen_pwy.remove(pwy)
